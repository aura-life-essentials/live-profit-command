...
import { ShopifyProduct, storefrontApiRequest, SHOPIFY_CONFIG } from '@/lib/shopify';
...
async function removeLineFromShopifyCart(cartId: string, lineId: string): Promise<{ success: boolean; cartNotFound?: boolean }> {
  try {
    ...
    console.error('Failed to remove line from cart:', error);
    return { success: false };
  }
}
          ...
          if (!cartId) {
            ...
            if (result.success) {
              ...
              toast.success('Cart updated', {
                description: `${item.product.node.title} x${newQuantity}`,
              });
